...
#
# Mark.G.Naumowicz@verizon.com  516.797.3803  04/23/2003
##########################################################
...
$filedir        = "/f5v1/ndmrecv/prod/$jobname/$daystamp";
$srcfile	= $filename1;
# ---------------------------------------------------------------------------------------------------
...
$dsn = "DBI:Sybase:new_licdns03:1433";
...
$stmt = "INSERT INTO stats \( source_file, filename, jobname, filesize, path , pid\) VALUES \(\"$f_name\", \"$filename\", \"$jobname\", \"$filesize\", \"$filepath\", \"$pid\"\)";
...
require Mail::Send;
    ...
    $msg = new Mail::Send;
    $msg->to('litech@core.verizon.com');
    $msg->subject('Unixmover alert message');
    $msg->set('From', 'Unixmover');
...
require Mail::Send;
    ...
    $msg->set('From', 'Unixmover');
    $fh = $msg->open;               # some default mailer
    print $fh "Could not move/backup file - $_[0]";
    $fh->close;         # complete the message and send it
}

sub check_stats {

($garbage,$s,$n,$p,$j,$dt,$fl) = split ("/", $destfile);
